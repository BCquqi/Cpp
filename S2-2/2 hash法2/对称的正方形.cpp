...
unsigned long long a[N][N],h[N][N],h1[N][N],h2[N][N],p[N],q[N];
int n,m,ans = 0;
...
bool check(int x1,int y1,int x2,int y2) {
    return Hash(h,x1,y1,x2,y2) == Hash(h1,n - x2 + 1,y1,n - x1 + 1,y2) && Hash(h,x1,y1,x2,y2) == Hash(h2,x1,m - y2 + 1,x2,m - y1 + 1);
}
...
int main() {
    ...
    for (int i = 1;i <= n;i++)
        for (int j = 1;j <= m;j++)
            ...
            h1[i][j] = h1[i - 1][j] * P + h1[i][j - 1] * Q - h1[i - 1][j - 1] * P * Q + a[n - i + 1][j],
            h2[i][j] = h2[i - 1][j] * P + h2[i][j - 1] * Q - h2[i - 1][j - 1] * P * Q + a[i][m - j + 1];
    for (int i = 1;i <= n;i++)
        for (int j = 1;j <= m;j++) {
            int L = 0,R = min(min(i - 1,j - 1),min(n - i,m - j)),len = 0;
            while (L <= R) {
                int mid = L + R >> 1;
            ...
            ans += len + 1;
            L = 1,R = min(min(n - i,m - j),min(i,j)),len = 0;
            while (L <= R) {
                int mid = L + R >> 1;
